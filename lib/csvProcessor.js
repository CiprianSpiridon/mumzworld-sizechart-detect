...
function processCSVStream(readStream, outputPath, resolve, reject) {
    ...
    const stringifier = stringify({
        ...
        columns: {
            ...
            'image 1 confidence': 'image 1 confidence',
            'image 2 confidence': 'image 2 confidence',
            'image 3 confidence': 'image 3 confidence'
        }
    });

    // Helper function to process a single image
    async function processImage(imageUrl, index, tempFiles) {
        if (!imageUrl) return { answer: 'ERROR - No image', confidence: 0 };
        ...
        try {
            ...
            if (error.message === 'Not a valid image file') {
                return { answer: 'ERROR - Not a valid image', confidence: 0 };
            }
            return { answer: 'ERROR - ' + error.message, confidence: 0 };
        }
    }
        ...
        async transform(row, encoding, callback) {
            try {
                ...
                const processedRow = {
                    ...
                    'image 2 result': '',
                    'image 3 result': '',
                    'image 1 confidence': 0,
                    'image 2 confidence': 0,
                    'image 3 confidence': 0
                };
                ...
                try {
                    ...
                    // Add results and confidences
                    results.forEach((result, index) => {
                        if (typeof result === 'object') {
                            processedRow[`image ${index + 1} result`] = result.answer;
                            processedRow[`image ${index + 1} confidence`] = result.confidence;
                        } else {
                            processedRow[`image ${index + 1} result`] = result;
                            processedRow[`image ${index + 1} confidence`] = 0;
                        }
                    });

                    // Count errors
                    if (results.some(r => r && (typeof r === 'string' ? r.startsWith('ERROR') : r.answer.startsWith('ERROR')))) {
                        errorCount++;
                    }
                } catch (error) {
                    ...
                    processedRow['image 1 confidence'] = 0;
                    processedRow['image 2 confidence'] = 0;
                    processedRow['image 3 confidence'] = 0;
                    errorCount++;
                } finally {
